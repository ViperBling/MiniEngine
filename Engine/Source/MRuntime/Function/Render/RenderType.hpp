...
namespace MiniEngine
{
    enum class MINIENGINE_IMAGE_TYPE : uint8_t
    {
        MINIENGINE_IMAGE_TYPE_UNKNOWM = 0,
        MINIENGINE_IMAGE_TYPE_2D
    };
    ...
    public:
        ...
        RHIFormat          mFormat = RHI_FORMAT_MAX_ENUM;
        MINIENGINE_IMAGE_TYPE mType {MINIENGINE_IMAGE_TYPE::MINIENGINE_IMAGE_TYPE_UNKNOWM};
        ...
        bool isValid() const { return mPixels != nullptr; }
    };
    ...
    struct MeshSourceDesc
    {
        ...
        bool   operator==(const MeshSourceDesc& rhs) const { return mMeshFile == rhs.mMeshFile; }
        size_t GetHashValue() const { return std::hash<std::string> {}(mMeshFile); }
    };

    struct MaterialSourceDesc
    {
        std::string mBaseColorFile;
        std::string mMetallicRoughnessFile;
        std::string mNormalFile;
        std::string mOcclusionFile;
        std::string mEmissiveFile;

        bool operator==(const MaterialSourceDesc& rhs) const
        {
            return mBaseColorFile == rhs.mBaseColorFile &&
                mMetallicRoughnessFile == rhs.mMetallicRoughnessFile && mNormalFile == rhs.mNormalFile &&
                mOcclusionFile == rhs.mOcclusionFile && mEmissiveFile == rhs.mEmissiveFile;
        }
        ...
        size_t GetHashValue() const
        {
            size_t hash = 0;
            HashCombine(
                hash, mBaseColorFile, mMetallicRoughnessFile, mNormalFile, mOcclusionFile, mEmissiveFile);
            return hash;
        }
    };
    ...
    {
        std::shared_ptr<BufferData> mVertexBuffer;
        std::shared_ptr<BufferData> mIndexBuffer;
    };

    struct RenderMeshData
    {
        StaticMeshData              mStaticMeshData;
        std::shared_ptr<BufferData> mSkeletonBindingBuffer;
    };

    struct RenderMaterialData
    {
        std::shared_ptr<TextureData> mBaseColorTexture;
        std::shared_ptr<TextureData> mMetallicRoughnessTexture;
        std::shared_ptr<TextureData> mNormalTexture;
        std::shared_ptr<TextureData> mOcclusionTexture;
        std::shared_ptr<TextureData> mEmissiveTexture;
    };
} // namespace MiniEngine

template<>
struct std::hash<MiniEngine::MeshSourceDesc>
{
    size_t operator()(const MiniEngine::MeshSourceDesc& rhs) const noexcept { return rhs.GetHashValue(); }
};
template<>
struct std::hash<MiniEngine::MaterialSourceDesc>
{
    size_t operator()(const MiniEngine::MaterialSourceDesc& rhs) const noexcept { return rhs.GetHashValue(); }
};
