...
namespace MiniEngine
{
    ...
    {
        mMat[0][col_index] = vec.x;
        mMat[1][col_index] = vec.y;
        mMat[2][col_index] = vec.z;
    }
    //-----------------------------------------------------------------------
    ...
    void Matrix3x3::CalculateQDUDecomposition(Matrix3x3& out_Q, Vector3& out_D, Vector3& out_U) const
    {
        ...
        float inv_length = mMat[0][0] * mMat[0][0] + mMat[1][0] * mMat[1][0] + mMat[2][0] * mMat[2][0];
        if (!Math::RealEqual(inv_length, 0))
            inv_length = Math::InvSqrt(inv_length);

        out_Q[0][0] = mMat[0][0] * inv_length;
        out_Q[1][0] = mMat[1][0] * inv_length;
        out_Q[2][0] = mMat[2][0] * inv_length;

        float dot   = out_Q[0][0] * mMat[0][1] + out_Q[1][0] * mMat[1][1] + out_Q[2][0] * mMat[2][1];
        out_Q[0][1] = mMat[0][1] - dot * out_Q[0][0];
        out_Q[1][1] = mMat[1][1] - dot * out_Q[1][0];
        out_Q[2][1] = mMat[2][1] - dot * out_Q[2][0];
        ...
        dot         = out_Q[0][0] * mMat[0][2] + out_Q[1][0] * mMat[1][2] + out_Q[2][0] * mMat[2][2];
        out_Q[0][2] = mMat[0][2] - dot * out_Q[0][0];
        out_Q[1][2] = mMat[1][2] - dot * out_Q[1][0];
        out_Q[2][2] = mMat[2][2] - dot * out_Q[2][0];
        dot         = out_Q[0][1] * mMat[0][2] + out_Q[1][1] * mMat[1][2] + out_Q[2][1] * mMat[2][2];
        ...
        Matrix3x3 R;
        R[0][0] = out_Q[0][0] * mMat[0][0] + out_Q[1][0] * mMat[1][0] + out_Q[2][0] * mMat[2][0];
        R[0][1] = out_Q[0][0] * mMat[0][1] + out_Q[1][0] * mMat[1][1] + out_Q[2][0] * mMat[2][1];
        R[1][1] = out_Q[0][1] * mMat[0][1] + out_Q[1][1] * mMat[1][1] + out_Q[2][1] * mMat[2][1];
        R[0][2] = out_Q[0][0] * mMat[0][2] + out_Q[1][0] * mMat[1][2] + out_Q[2][0] * mMat[2][2];
        R[1][2] = out_Q[0][1] * mMat[0][2] + out_Q[1][1] * mMat[1][2] + out_Q[2][1] * mMat[2][2];
        R[2][2] = out_Q[0][2] * mMat[0][2] + out_Q[1][2] * mMat[1][2] + out_Q[2][2] * mMat[2][2];
    ...
    void Matrix3x3::ToAngleAxis(Vector3& axis, Radian& radian) const
    {
        ...
        float trace = mMat[0][0] + mMat[1][1] + mMat[2][2];
        ...
        if (radian > Radian(0.0))
        {
            ...
            else
            {
                // angle is PI
                float half_inv;
                if (mMat[0][0] >= mMat[1][1])
                {
                    // r00 >= r11
                    if (mMat[0][0] >= mMat[2][2])
                    {
                        // r00 is maximum diagonal term
                        axis.x   = 0.5f * Math::Sqrt(mMat[0][0] - mMat[1][1] - mMat[2][2] + 1.0f);
                        half_inv = 0.5f / axis.x;
                        axis.y   = half_inv * mMat[0][1];
                        axis.z   = half_inv * mMat[0][2];
                    }
                    else
                    {
                        // r22 is maximum diagonal term
                        axis.z   = 0.5f * Math::Sqrt(mMat[2][2] - mMat[0][0] - mMat[1][1] + 1.0f);
                        half_inv = 0.5f / axis.z;
                        axis.x   = half_inv * mMat[0][2];
                        axis.y   = half_inv * mMat[1][2];
                    }
                }
                else
                {
                    // r11 > r00
                    if (mMat[1][1] >= mMat[2][2])
                    {
                        // r11 is maximum diagonal term
                        axis.y   = 0.5f * Math::Sqrt(mMat[1][1] - mMat[0][0] - mMat[2][2] + 1.0f);
                        half_inv = 0.5f / axis.y;
                        axis.x   = half_inv * mMat[0][1];
                        axis.z   = half_inv * mMat[1][2];
                    }
                    else
                    {
                        // r22 is maximum diagonal term
    ...
    void Matrix3x3::FromAngleAxis(const Vector3& axis, const Radian& radian)
    {
        ...
        mMat[0][0] = x2 * one_minus_cos + cos_v;
        mMat[0][1] = xym - z_sinv;
        mMat[0][2] = xzm + y_sin_v;
        mMat[1][0] = xym + z_sinv;
        mMat[1][1] = y2 * one_minus_cos + cos_v;
        mMat[1][2] = yzm - x_sin_v;
        mMat[2][0] = xzm - y_sin_v;
        mMat[2][1] = yzm + x_sin_v;
        mMat[2][2] = z2 * one_minus_cos + cos_v;
    }
} // namespace MiniEngine
