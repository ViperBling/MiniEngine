...
namespace MiniEngine
{
    EditorTranslationAxis::EditorTranslationAxis()
    {
        ...
        mMeshData.mStaticMeshData.mVertexBuffer = std::make_shared<BufferData>(vertex_data_size);
        uint8_t* vertex_data = static_cast<uint8_t*>(mMeshData.mStaticMeshData.mVertexBuffer->mData);
        ...
        mMeshData.mStaticMeshData.mIndexBuffer = std::make_shared<BufferData>(index_data_size);
        uint16_t* index_data = static_cast<uint16_t*>(mMeshData.mStaticMeshData.mIndexBuffer->mData);
